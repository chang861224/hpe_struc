...
void proNet::LoadEdgeList(string filename, bool undirect, string filename_s, int dimensions) {
    ...
    if(filename_s.length()){
        ...
        // Revise edge-weight from structural embedding
        for (int i=0; i<filenames.size();i++)
        {
            fin = fopen(filenames[i].c_str(), "rb");
            for (line = 0 ; line != filelines[i]; line++)
            {
                if(fscanf(fin, "%s %s %lf", v1, v2, &w) != 3){
                    cout << "\t[ERROR] line " << line << " contains wrong number of column data" << endl;
                    continue;
                ...
                vector<double> e1 = weight_struct[vid1];
                vector<double> e2 = weight_struct[vid2];
                int idx = find(graph[vid1].begin(), graph[vid1].end(), vid2) - graph[vid1].begin();

                edge[vid1][idx] = dot(e1, e2);
            }
            fclose(fin);
        }
    }
